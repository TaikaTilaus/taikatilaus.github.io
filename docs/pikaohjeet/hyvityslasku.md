---
sidebar_position: 5
---

# Hyvityslasku asiakkaalle

### 1. Etusivunäkymä

![1. Etusivunäkymä](/img/pikaohjeet/hyvityslasku1.png)

### 2. Etusivunäkymän hausta haetaan lasku valitulla hakusanalla

![2. Etusivunäkymän haku](/img/pikaohjeet/hakuehto.png)

### TAI 2. Laskut -valikosta Laskujen haku -välilehdestä haetaan lasku laskunumerolla

![2. Laskut - välilehden haku](/img/pikaohjeet/hyvityslasku3.png)

### 3. Avaa lasku klikkaamalla laskun numeroa
![3. Laskut -hakutulos](/img/pikaohjeet/hyvityslasku4.png)

### 4. Laskunäkymän alapalkista painetaan Luo hyvityslasku-painiketta
![4. Laskunäkymä](/img/pikaohjeet/hyvityslasku6.png)

### 5. Hyvityslaskun luonti vahvistetaan
![5. Hyvityslaskun luonnin vahvistus](/img/pikaohjeet/hyvityslasku5.png)

### 6. Kun hyvityslasku on luotu, voit lähettää sen asiakkaalle avaamalla hyvityslaskun tiedot Kynä-ikonista

Tämä vaihe on valinnainen, ja se täytyy tehdä vain, jos asiakas haluaa kopion hyvityslaskusta esim. kirjanpitoa varten. 

![5. Hyvityslaskun luonnin vahvistus](/img/pikaohjeet/hyvityslasku7.png)

### 7. Aktivoi Lähetys sallittu -kenttä. Kirjaa Laskun saa lähettää päivämäärä. Tallenna. 

![5. Hyvityslaskun luonnin vahvistus](/img/pikaohjeet/hyvityslasku8.png)

### 8. Jos laskujen lähetys on automatisoitu, lasku lähtee asiakkaalle automaattisesti. Jos laskutus on manuaalista, lähetä lasku matkaan Laskujen lähetys -välilehdeltä.